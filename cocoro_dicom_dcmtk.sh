...
summary=summary_${timestamp2}.txt

dcmdump `ls | head -1 ` | sed -e 's/  *#.*$//' -e 's/^  *//' -e 's/^  *$//' | sed -e 's/=//g' -e 's/\[//' -e 's/\]//' > tmp

echo "確認日:			${timestamp}" > $summary

temp=$(grep '0008,0080' tmp | cut -c 16-)
institution=${temp}
echo "施設名:			${institution}" >> $summary

temp=$(grep '0020,0011' tmp | cut -c 16-)
seq_no=${temp}
echo "シークエンス番号:	${seq_no}" >> $summary

temp=$(grep '0008,0070' tmp | cut -c 16-)
vendor=${temp}
echo "メーカー:		${vendor}" >> $summary

temp=$(grep '0008,1090' tmp | cut -c 16-)
model=${temp}
echo "機種名:			${model}" >> $summary

temp=$(grep '0018,0087' tmp | cut -c 16-)
mag_field_strength=${temp}
echo "磁場強度:		${mag_field_strength}T" >> $summary

temp=$(grep '0018,1020' tmp | cut -c 16-)
software_ver=${temp}
echo "システムバージョン:	${software_ver}" >> $summary

temp=$(grep '0018,1250' tmp | cut -c 16-)
head_coil=${temp}
echo "Head_Coil:		N/A" >> $summary

temp=$(grep '0008,103e' tmp | cut -c 16-)
pulse_sequence=${temp}
...
temp=$(grep '0018,0080' tmp | cut -c 16-)
tr=${temp}
echo "TR:			${tr}" >> $summary

temp=$(grep '0018,0081' tmp | cut -c 16-)
te=${temp}
echo "TE:			${te}" >> $summary

temp=$(grep '0018,0082' tmp | cut -c 16-)
ti=${temp}
echo "TI:			${ti}" >> $summary

temp=$(grep '0018,1314' tmp | cut -c 16-)
flip_angle=${temp}
echo "Flip_Angle:		${flip_angle}" >> $summary

temp=$(grep '0018,1310' tmp | cut -c 16-)
matrix=${temp}
echo "Matrix:			${matrix}" >> $summary

temp=$(grep '0028,0010' tmp | cut -c 16-)
row=${temp}
echo "Row:			${row}" >> $summary

temp=$(grep '0028,0011' tmp | cut -c 16-)
column=${temp}
echo "Column:			${column}" >> $summary

temp=$(grep '0028,0030' tmp | cut -c 16-)
pixelspacing=${temp}
echo "PixelSpacing:		${pixelspacing}" >> $summary

temp=$(grep '0018,0050' tmp | cut -c 16-)
slicethickness=${temp}
...
#temp=$(grep '0020,0037' tmp | cut -c 16-)
#slice_orientation=${temp}
echo "Slice_orientation:	N/A" >> $summary
#
temp=$(grep '0018,1312' tmp | cut -c 16-)
slice_encoding=${temp}
